      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-4">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          {isPreview ? (
            ...
            <Link to="/" className="cursor-pointer">{LogoContent}</Link>
          )}
          ...
          <form 
            ...
            className="flex-1 max-w-2xl w-full flex items-center gap-3"
          >
      ...
      <div className="bg-white border-t border-slate-100 hidden md:block">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <nav className="flex items-center gap-1">
            {isPreview ? (
               <div className="px-4 py-3.5 text-[13px] font-bold uppercase tracking-wider text-slate-400 select-none cursor-default">Trang chủ</div>
            ) : (
              <Link 
                to="/" 
                className={`px-4 py-3.5 text-[13px] font-bold uppercase tracking-wider border-b-2 transition-all ${!selectedParentCategory ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
              >
                Trang chủ
              </Link>
            )}
              ...
              return (
                <div key={pc.id} className="relative group">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-3.5 text-[13px] font-bold uppercase tracking-wider border-b-2 transition-all ${isActive ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
                  >
                    {pc.name}
                    {subCats.length > 0 && <ChevronDown size={14} className={`transition-transform duration-300 ${isActive ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-800'} group-hover:rotate-180`} />}
                  </button>
            ...
            {isPreview ? (
              <div className="px-4 py-3.5 text-[13px] font-bold uppercase tracking-wider text-slate-400 select-none cursor-default">Tất cả dự án</div>
            ) : (
              <Link 
                to="/category" 
                className={`px-4 py-3.5 text-[13px] font-bold uppercase tracking-wider border-b-2 transition-all ${!selectedParentCategory && window.location.pathname === '/category' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
              >
                Tất cả dự án
              </Link>
            )}
